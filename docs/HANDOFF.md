...
Nessun'altra feature funzionale è in coda: la prossima è la prima idea v2.0 qui sotto (canali user-owned), da pianificare.

**Idee UX per una futura v2.0 (valutate 2026-07-07):**

- ⏭️ **Canali di lega user-owned — PROSSIMA FEATURE DA PIANIFICARE** (priorità aggiornata da Matteo il 2026-07-07, scavalca l'ordine indicativo originale). Un utente aggiunge il bot come admin del proprio canale Telegram (fuori dal canale ufficiale @fantaformazionireminders) per ricevere lì i promemoria della propria lega. Contesto utile per la pianificazione:
...
- **Inline keyboard (bottoni callback)** al posto dei soli comandi: `/start` in privato con bottone "Attiva promemoria", `/promemoria` con toggle on/off, scelta degli orari a bottoni. È solo codice PTB (`InlineKeyboardMarkup` + `CallbackQueryHandler`), nessun setting BotFather. Rimandata dopo i canali.
...
**Stato al momento dell'handoff (agg. 2026-07-07, fine sessione feature #1–#3):** **prod = v0.11.0** (rilasciata il 2026-07-07 via Prepare release + merge PR #18), `dev` e `main` allineati, nessuna PR aperta. Tutte e tre le feature della roadmap funzionale sono in produzione. BotFather aggiornato su entrambi i bot. Azioni pendenti non bloccanti: aggiungere il secret `FOOTBALL_DATA_API_KEY`; la migrazione VM resta bloccata da mancanza di capacità A1.Flex lato Oracle (nessuna azione nostra possibile, solo ritentare). Nessun problema noto sul codice.

**Prossimo lavoro pianificato**: design + implementazione dei **canali di lega user-owned**. Il contesto tecnico rilevante (perché serve `my_chat_member`, cosa nello schema è già pronto, cosa manca, le 4 decisioni aperte) è **già raccolto nella sezione "Idee UX per una futura v2.0" sopra — parti da lì, non serve riderivarlo leggendo il codice da zero.** ADR 0008 (+ amendment) e ADR 0012 sono citate lì solo come riferimento per i dettagli implementativi (schema `origin`, pattern admin-only) che la sintesi sopra non riporta per esteso — leggerle non sostituisce la sezione, la completa.

Segui lo stile ADR-first: usa AskUserQuestion sulle 4 decisioni aperte elencate sopra (e su qualunque altro trade-off emerga) prima di finalizzare il design; scrivi una nuova ADR (0015) con le decisioni prese prima di implementare.

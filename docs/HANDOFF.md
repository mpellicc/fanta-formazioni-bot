...
**Stato al momento dell'handoff (agg. 2026-07-08):** **prod = v0.12.0**, `dev`/`main` allineati. Le tastiere inline (ADR 0015) sono in prod da v0.12.0. Sessione corrente: branch **`feature/mock-calendar-provider`** (da `dev`) con il provider `mock` (ADR 0016) implementato — `calendar/mock.py`, `CalendarProvider.MOCK` + `mock_kickoff_offset` in `config.py`, case nella factory `calendar/base.py`, wiring `MOCK_KICKOFF_OFFSET` in `deploy.yml` (var per-environment, default `10m`) — più la **riconciliazione di tutta la documentazione allo stato corrente** e il rename `env.example` → `.env.example` (con `.gitignore` aggiornato a `!.env.example`). `ruff`/`ruff format`/`mypy`/`pytest` (69 test) verdi. **PR #21** aperta verso `dev`: Matteo la mergia con **squash** dopo aver rimosso le env var `CALENDAR_PROVIDER=mock`/`MOCK_KICKOFF_OFFSET` dall'environment `development` su GitHub (altrimenti il bot dev resterebbe sul provider mock). Azioni pendenti non bloccanti ereditate: secret `FOOTBALL_DATA_API_KEY` da aggiungere; migrazione VM ad A1.Flex bloccata da mancanza di capacità Oracle.

**Prossimo lavoro pianificato**: 1) **Roadmap verso la v1.0** (obiettivo: prod entro l'inizio campionato, 22 ago 2026) — revisione testi/formatting dei messaggi + giro di test manuali strutturati sul bot dev, con dentro il fix del bug "Salva sovrascrive i custom" e il display sempre-sincronizzato degli orari custom (bug e nice-to-have sono la stessa radice; dettagli nella sezione "Roadmap verso la v1.0" sopra). 2) Dopo la 1.0, design + implementazione dei **canali di lega user-owned**: il contesto tecnico (perché serve `my_chat_member`, cosa nello schema è già pronto, cosa manca, le 4 decisioni aperte) è nella sezione "Idee UX per una futura v2.0" sopra — parti da lì. ADR 0008 (+ amendment) e ADR 0012 come riferimento implementativo (schema `origin`, pattern admin-only). Stile ADR-first: AskUserQuestion sulle 4 decisioni aperte prima del design, **nuova ADR 0017** (la 0016 è già del provider mock) prima di implementare. Il **dashboard/pannello** resta un'esplorazione futura non pianificata, fuori dalla 1.0 (vedi sezione idee).

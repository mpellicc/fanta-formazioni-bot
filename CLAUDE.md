...
- **Seasonal maintenance**: each minor gets a long-running `release-X.Y` branch, cut from `main` at its tag, for in-season bugfixes only (patch tags `vX.Y.z`) — currently **`release-1.1`**. Fix on `main` first, then cherry-pick to the release branch and tag the patch — never fix on the release branch first. A minor can still ship mid-season straight from `main`: production follows the tag, not the branch (ADR 0017 + amendment).
